...
author: Maurice Farr
date: 12/9/2024
assignment: SDEV-140 GUI Tkinter App Final Project, Part 2
...
#Define confirmation message for the buttons (create_account_button and login_button)
cnca_message1 = "ERROR: Please fill out the required information to complete this process"
cnca_message2 = "     Congrats, you have officially created an account. You can now use your created account to login."
...
#Define the function to open a new window to display the message that new account has been created including adding grid layouts and the parameters
def display_confirmation_message(window_combine, entry_email1, entry_username, entry_password1, display_message1, display_message2):
    ...
    #Conditional if-else statement to check if there are any entries filled in the entry box for creating new account
    if len(email) != 0 and len(username) != 0 and len(password) != 0: #Indication that no entries are filled out in the entry box
        window_cnca = Toplevel()
        window_cnca.geometry("700x65")
        ...
        window_cnca.after(3000, lambda: destroyWindow(window_cnca))  # Indication that the cnca_message1 will display for 3 seconds before disappearing
        ...
        cnca_label.grid(row=28, column=3, padx=20, pady=20)
        window_cnca.after(3000, lambda: destroyWindow(window_cnca))  # Indication that the cnca_message2 will display for 3 seconds before disappearing
        window_combine.destroy()  # Both windows (window_n_a and window_cnca) will disappear
...
def login_process(username_entrybox, password_entrybox, window_login):
    ...
    if len(entries) == 0: #indication that there are missing or invalid info into entry text field when logging in
        window_error1 = Toplevel()
        window_error1.geometry("300x65")
        window_error1.config(bg="#ffffff")
        error1_label = Label(window_error1, text= login_message1, font=("Californian FB", 12, "bold"), fg="#1197e0",bg="#ffffff")
        error1_label.grid(row=28, column=3, padx=20, pady=20)
        window_error1.after(3000, lambda: destroyWindow(window_error1))  # Indication that the login_message1 will display for 3 seconds before disappearing
        window_login.destroy()  # Window_login will disappear after failing to login
    else:
        #Indication that the user was able to logged in
        window_login_succces = Toplevel()
        window_login_succces.geometry("200x65")
        window_login_succces.config(bg="#ffffff")
        window_login_succces_label = Label(window_login_succces, text=login_message2, font=("Californian FB", 12, "bold"), fg="#1197e0",bg="#ffffff")
        window_login_succces_label.grid(row=28, column=3, padx=20, pady=20)
        window_login_succces.after(3000, lambda: destroyWindow(window_login_succces))  # Indication that the login_message2 will display for 3 seconds before disappearing
        window_login.destroy()  # Window_login will disappear after logging
...
def submit_blood_glucose(window_combine, user_glucose):
    if len(user_glucose) == 0:  #Check to see if there are any empty entries for entering blood glucose level
        window_error2 = Toplevel()
        window_error2.geometry("490x60")
        window_error2.config(bg="#ffffff")
        error2_label = Label(window_error2, text="     ERROR: Please enter your blood glucose level!",font=("Californian FB", 12, "bold"), fg="#1197e0", bg="#ffffff")
        ...
        if user_glucose < 0: #Determine if blood glucose level is less than zero
            ...
            error3_label = Label(window_error3, text="     ERROR: You cannot have a negative value!", font=("Californian FB", 12, "bold"), fg="#1197e0", bg="#ffffff")
        ...
        elif user_glucose != int(user_glucose):  #Determine if blood glucose level is a decimal number
            window_error4 = Toplevel()
            window_error4.geometry("455x60")
            window_error4.config(bg="#ffffff")
            error4_label = Label(window_error4, text="      ERROR: You cannot have a decimal value!", font=("Californian FB", 12, "bold"), fg="#1197e0", bg="#ffffff")
            error4_label.grid(row=28, column=3, padx=20, pady=20)
            window_error4.after(3000, lambda: destroyWindow(window_error4))  #The "error_4" message will display for 3 seconds before disappearing
        else:
...
def update_password(window_combine, path_for_file, email_entrybox, new_password_entrybox, retype_new_password_entrybox):
        ...
        #Conditional if-else statements for resetting new password
        if (len (new_password) != 0 #Check if there is an entry to type new password
        and len (retype_new_password) != 0 #Check if there is an entry to retype new password for confirmation
        and new_password == retype_new_password): #Check if the password that is typed in both entrybox (the new_password and retype_new_password) matches with each other
            ...
            #Create a window and adding grid layouts to display the message that the new password has been created
            window_npw = Toplevel()
            ...
            window_npw.after(3000, lambda: destroyWindow(window_npw))  #The new password confirmation will display for 3 seconds before disappearing
            window_combine.destroy()  #Both windows (window_frp and window_npw) will disappear
        else: #Create a window and adding grid layouts to display an error message while creating new password
            window_npw = Toplevel()
            window_npw.geometry("330x65")
            window_npw.config(bg="#ffffff")
            pw_label = Label(window_npw,text="ERROR: Please fill out or correct any entries!",font=("Californian FB", 12, "bold"), fg="#1197e0", bg="#ffffff")
            pw_label.grid(row=28, column=3, padx=20, pady=20)
            window_npw.after(3000, lambda: destroyWindow(window_npw))  #The error message will display for 3 seconds before disappearing
            window_combine.destroy()  #Both windows (window_frp and window_npw) will disappear
